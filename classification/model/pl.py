...
class PLModel(pl.LightningModule):
    ...
    def aggregate_preds_for_patients(
            ...
            mode : Literal["mean", "map"] = "max",
            ) -> torch.Tensor:
        mapping = {}
        for i, p in enumerate(patient_inds):
            mapping.setdefault(p, []).append(i)

        preds_agg = [preds[i] for i in mapping.values()]
        if mode == "mean":
            preds_agg = [p.mean(0) for p in preds_agg]
        elif mode == "max":
            preds_agg = [p.max(0).values for p in preds_agg]
        else:
    ...
    def aggregate_scores_for_patients(self,
        ...
        preds_agg = self.aggregate_preds_for_patients(
            preds, patient_inds, mode=self.aggregate_mode)
        labels_agg = self.aggregate_preds_for_patients(
            labels, patient_inds, mode="max")
