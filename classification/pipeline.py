...
from denoising.data_utils import apply_canny
...
class ClassificationPipeline:
    def __init__(
        ...
        denoising_checkpoint: Optional[
            Union[str, os.PathLike]
        ] = "res/checkpoints/denoiser/best.ckpt",
        ...
        filename_checkpoint = _folder_checkpoints.joinpath(
            model_name, f"{checkpoint_name}.ckpt"
        )
        ...
        if seq_model_name is None:
            ...
            filename_checkpoint_seq = _folder_checkpoints.joinpath(
                seq_model_name, "best.ckpt"
            )
        ...
        if segm_model_name is None:
            ...
            filename_params_segm = _folder_models.joinpath(
                segm_model_name, "params.json"
            )
            filename_checkpoint_segm = _folder_checkpoints.joinpath(
                segm_model_name, "best.ckpt"
            )
    ...
    def predict(
        ...
        if (
            (z_positions is None and self.mode.startswith("3d")) or patient_ids is None
        ) and folder is not None:
            dcm_data = self.extract_data_from_folder(
                folder, ids=ids, threads=self.threads
            )
        ...
        if images is not None and all(
            isinstance(img, pydicom.FileDataset) for img in images
        ):
            if z_positions is None:
        ...
        with torch.no_grad():
            ...
            for batch in Progress(
                ...
                if self.mode.startswith("2d"):
                    ...
                    p_index = np.concatenate(
                        [[p] * len(i) for i, p in zip(sop_index, p_index)]
                    )
                    p_id = np.concatenate(
                        [[p] * len(i) for i, p in zip(sop_index, p_id)]
                    )
                ...
                if self.denoiser is not None:
                    ...
                    for img in images:
                        ...
                        edge_channel.append(edges)
                    edge_channel = torch.cat(edge_channel)
                    edge_channel = edge_channel.unsqueeze(1)
                    img_w_edges = torch.cat([images, edge_channel], dim=1)
                    img_w_edges = img_w_edges.float()
                    pred = self.denoiser.generator(img_w_edges)

                    # difference smoothed
                    pred = torch.abs(pred - images)
                    pred = self.post_process_image(pred)
                    pred[pred < 0.1] = 0.0
                    # pred[pred > 0.1] = 1.0
                    # pred = self.post_process_image(pred)
                    ...
                    denoised_images.append(pred.cpu())
            ...
            if denoised_images:
                denoised_images = torch.cat(denoised_images).numpy()
        ...
        if segm_masks is not None:
            segm_masks_final = np.zeros(
                (n_images,) + tuple(segm_masks.shape[1:]), np.float32
            )
        ...
        if denoised_images is not None:
            ...
            denoised_images_final[sop_inds_all] = denoised_images
        ...
        if get_full_data:
            ...
            if self.denoiser is not None:
                result.update(
                    {
                        "denoised_images": denoised_images_final,
                    }
                )
            return result
        else:
